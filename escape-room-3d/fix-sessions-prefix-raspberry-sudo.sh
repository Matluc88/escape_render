...
# Fix Router Prefix in sessions.py - Raspberry Pi (con sudo)
# Corregge prefix da "/sessions" a "/api/sessions"

echo "🔧 FIX ROUTER PREFIX /api/sessions (con sudo)"
echo "=============================================="
echo ""
...
if [ -f "backend/app/api/sessions.py" ]; then
    sudo cp backend/app/api/sessions.py backend/app/api/sessions.py.backup.$(date +%Y%m%d-%H%M%S)
    echo "✅ Backup creato"
else
...
cat backend/app/api/sessions.py | grep 'router = APIRouter'
echo ""
