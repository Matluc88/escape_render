/* =====================================================
   ...
   🔧 FIX: LED Porta Blinking corretto + WORKAROUND endpoint!
   
   WORKAROUND: Usa /api/sessions/{id}/game-completion/state
   invece di /api/game-completion/door-leds (che non esiste nel backend vecchio)
...
void checkDoorLedState() {
  ...
  // 🔧 WORKAROUND: Usa endpoint con session_id invece di quello globale
  String endpoint = "/api/sessions/" + String(session_id) + "/game-completion/state";
  
  String response = getHTTP(endpoint.c_str());
  
  if (response.length() > 0) {
    StaticJsonDocument<2048> doc;  // Aumentato buffer per risposta più grande
    DeserializationError error = deserializeJson(doc, response);
    
    if (!error) {
      // Leggi door_led_states.soggiorno invece di root.soggiorno
      ...
      if (newState != nullptr) {
        ...
        Serial.println("⚠️ Campo 'soggiorno' non trovato in door_led_states");
      }
    } else {
      Serial.print("❌ JSON parse error: ");
      Serial.println(error.c_str());
...
void setup() {
  ...
  Serial.println("VERSION: Blinking FIX + WORKAROUND Endpoint 🔧");
  Serial.println("================================================\n");
  ...
  Serial.println("   🔧 WORKAROUND: Usa /api/sessions/{id}/game-completion/state");
